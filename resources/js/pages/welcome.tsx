    ...
    return (
        <div className="min-h-screen bg-slate-950 text-slate-200 selection:bg-indigo-500/30 selection:text-indigo-200 font-sans">
            ...
            <nav className="fixed top-0 z-50 w-full border-b border-white/5 bg-slate-950/70 backdrop-blur-xl">
                        ...
                        <div className="flex items-center gap-4">
                            {auth.user ? (
                                <Link
                                    ...
                                    className="rounded-full bg-slate-800 border border-slate-700 px-5 py-2 text-sm font-medium text-white hover:bg-slate-700 transition-all"
                                >
                                    Dashboard
                                </Link>
                            ) : (
                                <>
                                    ...
                                    <Link
                                        href={route('register')}
                                        className="rounded-full bg-indigo-600 px-5 py-2 text-sm font-medium text-white shadow-lg shadow-indigo-500/25 hover:bg-indigo-500 hover:shadow-indigo-500/40 transition-all"
                                    >
                                        Get Started
                                    </Link>
                                </>
                ...
                {/* Mobile Menu */}
                {isMenuOpen && (
                    <div className="lg:hidden absolute top-full left-0 w-full border-b border-white/5 bg-slate-950 px-6 py-6 shadow-2xl">
                        <div className="flex flex-col gap-4">
                            <a href='#hero' onClick={() => setIsMenuOpen(false)}  >Home</a>
                            <a href="#teams" onClick={() => setIsMenuOpen(false)} className="text-slate-300 hover:text-white">Teams</a>
                            <a href="#features" onClick={() => setIsMenuOpen(false)} className="text-slate-300 hover:text-white">Features</a>
                            {auth.user ? (
                                <Link href={route('dashboard')} className="text-center rounded-lg bg-indigo-600 py-3 font-medium text-white">Dashboard</Link>
                            ) : (
                                <div className="flex flex-col gap-3 mt-4">
                                    <Link href={route('login')} className="text-center rounded-lg border border-slate-700 py-2.5 font-medium text-slate-300">Log in</Link>
                                    <Link href={route('register')} className="text-center rounded-lg bg-indigo-600 py-2.5 font-medium text-white">Register</Link>
                                </div>
                            )}
                        </div>
                    </div>
            ...
            {/* Hero Section */}
            <section id='home'  className="relative z-10 flex min-h-[90vh] flex-col items-center justify-center px-6 pt-24 pb-16 text-center">
                <div className="animate-fade-in-up">
                    <h1 className="mt-8 max-w-4xl text-5xl font-extrabold tracking-tight text-white sm:text-7xl">
                        Level Up Your Learning with <br className="hidden sm:block" />
                    ...
                    <div className="mt-10 flex flex-col items-center justify-center gap-4 sm:flex-row">
                        <Link
                            href={route('register')}
                            className="group relative inline-flex h-12 items-center justify-center overflow-hidden rounded-full bg-indigo-600 px-8 font-medium text-white shadow-lg shadow-indigo-500/30 transition-all hover:bg-indigo-500 hover:scale-105 hover:shadow-indigo-500/50"
                        >
                ...
                <div className="mx-auto max-w-7xl px-6 lg:px-8">
                    ...
                    <div className="mx-auto mt-16 grid max-w-2xl grid-cols-1 gap-6 sm:grid-cols-2 lg:max-w-none lg:grid-cols-3 lg:gap-8">
                        {features.map((feature, index) => (
                            ...
                            >
                                <div className="mb-6 inline-flex h-12 w-12 items-center justify-center rounded-lg bg-slate-800 border border-white/5 shadow-inner">
                                    {feature.icon}
                                </div>
            ...
            <section id="teams" className="relative z-10 py-16">
                <div className="mx-auto max-w-7xl px-6 lg:px-8">
                    <div className="text-center">
                        <h2 className="text-3xl font-bold tracking-tight text-white sm:text-4xl">Know Who We Are</h2>
                        <p className="mt-4 text-lg text-slate-400">
                             We’re a passionate team of educators, developers, and
                        lifelong learners building a smarter way to grow your  skills. 
                         </p>
                        <p className='text-center text-lg text-slate-400'> our mission is to make education dynamic — where your
                    ...
                    {/* Team section  */}
                    <div className="mt-12 relative w-full overflow-hidden mask-gradient">
                        <div className="flex w-max animate-infinite-scroll hover:[animation-play-state:paused] gap-2">
                            {[...team, ...team, ...team].map((member, index) => (
                                <div
                                    key={index}
                                    className="group relative h-80 w-64 flex-shrink-0 flex flex-col items-center justify-center p-6 transition-all duration-300 hover:-translate-y-2"
                                >
                                    <div className="relative mb-6 h-32 w-32 rounded-full p-1 bg-gradient-to-br from-indigo-500 to-blue-600 shadow-lg shadow-indigo-500/20 group-hover:shadow-indigo-500/40 transition-shadow">
                                         <img
                                            src={member.image}
                                            alt={member.name}
                                            className="h-full w-full rounded-full object-cover border-4 border-slate-900"
                                        />
                                    </div>
            ...
            {/* Footer */}
            <footer id="footer" className="relative z-10 border-t border-white/5 bg-slate-950 pt-16 pb-8">
                <div className="mx-auto max-w-7xl px-6 lg:px-8">
                    <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
                        <div>
                            <div className="flex items-center gap-2 mb-4">
                                <span className="text-xl font-bold text-white">Ablex</span>
                            </div>
                            <p className="text-sm text-slate-400 leading-relaxed max-w-xs">
                                Empowering the next generation of learners with cutting-edge tools and a supportive community.
                            </p>
                        </div>
                        <div className="grid grid-cols-2 gap-8 lg:col-span-2 sm:grid-cols-3">
                            <div>
                                ...
                                <ul className="mt-4 space-y-2 text-sm text-slate-400">
                                    ...
                                    <li><a href="#teams" className="hover:text-indigo-400">Teams</a></li>
                                </ul>
                            </div>
                            <div>
                                <h3 className="text-sm font-semibold text-white">Connect</h3>
                                <ul className="mt-4 space-y-2 text-sm text-slate-400">
                                    <li className="flex items-center gap-2">Semara University</li>
